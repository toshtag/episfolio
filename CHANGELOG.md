...
多経路発想 / 求人票分析・余白設計 / 退職交渉設計・藩士意識のフィールドを既存 4 entity に拡張し、新 entity `ResignationPlan`（退職交渉タブ）を追加した機能リリース。

### Added

- **ResignationPlan エンティティ**（kernel / desktop）: 新 entity。内定比較表 7 項目（年収 / 年間休日 / 所定就業時間 / 通勤時間 / ポジション / 募集背景 / リスクメモ）、退職シーケンス 9 マイルストーン日付（最終面接〜入社）、退職交渉メモ（入社可能日レンジ・交渉メモ）、藩士意識メモ（失うもの / 得るもの / 次の出口戦略）を一元管理
- **退職交渉タブ**（desktop）: `resignation-plan-view` として main-view に追加。jobTarget（内定先）に紐づく複数プランを管理、マイルストーン日付入力・カード一覧・2 ステップ削除に対応
- **InterviewReport 余白設計・面接ログフィールド**（kernel / desktop）: 8 フィールドを追加。面接官属性（`interviewerRole` / `interviewerStyle`）、自己発話比率（`talkRatioSelf`）、質問内容メモ（`questionsAskedNote`）、手応え（`responseImpression`）、余白設計ノート（`blankAreasNote` / `improvementNote`）、合否（`passed` tristate）
- **AgentTrackRecord エージェント評価フィールド**（kernel / desktop）: 9 フィールドを追加。得意業界（`specialtyIndustries`）、得意職種（`specialtyJobTypes`）、コンサルタント品質（`consultantQuality`）、独占求人有無（`hasExclusiveJobs`）、推薦状対応（`providesRecommendationLetter` / `recommendationLetterReceived`）、紹介求人数（`numberOfJobsIntroduced`）、返信速度（`responseSpeedDays`）、総合評価（`overallRating`）
- **JobTarget 求人票分析フィールド**（kernel / desktop）: 16 フィールドを追加。年間休日（`annualHolidays`）、所定労働時間（`workingHoursPerDay`）、通勤時間（`commuteTimeMinutes`）、雇用形態（`employmentType`）、フレックス/リモート可否（`flexTimeAvailable` / `remoteWorkAvailable`）、有給取得平均（`averagePaidLeaveTaken`）、欠員理由（`vacancyReason`）、チーム規模（`currentTeamSize`）、給与形態・基本給・固定残業・賞与（`wageType` / `basicSalary` / `fixedOvertimeHours` / `bonusBaseMonths`）、昇給約束（`hasFutureRaisePromise` / `futureRaisePromiseInContract`）、応募経路（`applicationRoute`）

### Changed

- **migration 0034〜0037**（desktop / SQLite）: `job_targets` / `agent_track_records` / `interview_reports` に ALTER TABLE で拡張フィールドを追加、`resignation_plans` テーブルを新規作成
...
- v0.8 は完全ローカル動作のまま（外部送信 surface は v0.2.1 で物理削除済）
...
- v0.7 は完全ローカル動作のまま（外部送信 surface は v0.2.1 で物理削除済）
...
- v0.6 は完全ローカル動作のまま（外部送信 surface は v0.2.1 で物理削除済）
...
- v0.4 は完全ローカル動作のまま（外部送信 surface は v0.2.1 で物理削除済）
...
- v0.3 は完全ローカル動作のまま（外部送信 surface は v0.2.1 で物理削除済）
...
data integrity / test coverage patch。挙動変更なし、新機能なし、データ整合性とテストカバレッジ強化のみ。
...
v0.2.0 の security / integrity hotfix。

### Removed

- **AI 送信 / API key Tauri command を物理削除**: `save_api_key` / `load_api_key` / `delete_api_key` / `test_openai_connection` / `extract_evidence` / `generate_document` の 6 command が `invoke_handler` に登録されたままだった。UI hide では Tauri の権限境界として無意味（WebView から `invoke()` で直接呼べる）状態を、コード自体を削除することで物理的に閉じる
...
- **初期運用基盤**: hooks / skills / GitHub テンプレート / CI / ADR 0001〜0007
