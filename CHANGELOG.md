...
- **AgentTrackRecord エージェント評価フィールド**（kernel / desktop）: 9 フィールドを追加。強み把握力 / 求人提案精度 / 交渉支援力 / 全体評価（スコア）、担当 RA / CA の氏名・評価コメント・印象メモ、継続使用フラグ（`continueUsing`）
- **JobTarget 求人票分析フィールド**（kernel / desktop）: 16 フィールドを追加。必須要件 / 歓迎要件 / 仕事内容 / 組織規模 / ポジション / 報告先 / 採用背景 / 選考フロー / 待遇詳細、多経路アプローチメモ（`approachRoutes` / `multiPathNote`）
